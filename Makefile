...
docker_ui:
	docker build -t clusteroperator/ui:master  ./ui --no-cache

docker_server:
	docker build -t clusteroperator/server:master --build-arg GOPROXY=$(GOPROXY) --build-arg GOARCH=$(GOARCH) --build-arg XPACK="no" .

docker_server_xpack:
	docker build -t clusteroperator/server:master --build-arg GOPROXY=$(GOPROXY) --build-arg GOARCH=$(GOARCH) --build-arg XPACK="yes" .
